...
async def get_rental_trends():
    try:
        logger.info("Attempting to fetch rental trends data...")
        ...
        query = text("""
            ...
            ORDER BY 
                date;
        """)
        logger.debug(f"Executing rental data query: {query}")
        df = pd.read_sql(query, engine)
        logger.info(f"Query executed successfully. Row count: {len(df)}")
        logger.debug(f"DataFrame head: \n{df.head()}")

        if len(df) == 0:
            raise HTTPException(status_code=404, detail="No rental data found")
        ...
        error_msg = f"Error fetching rental trends: {str(e)}"
...
async def get_rental_growth():
    try:
        logger.info("Fetching rental growth rates data...")
        engine = get_db_connection()
        query_str = """
        ...
        logger.debug(f"Executing rental growth rates query: {query}")
        try:
            df = pd.read_sql(query, engine)
        ...
        error_msg = f"Error fetching rental growth rates: {str(e)}"
        logger.error(error_msg, exc_info=True)
        raise HTTPException(status_code=500, detail=error_msg)
...
async def get_rental_heatmap():
    try:
        logger.info("Fetching rental heatmap data...")
        engine = get_db_connection()
        query = text("""
            SELECT 
                TO_CHAR(date, 'YYYY-MM') as date,
            ...
            GROUP BY 
                date;
        """)
        logger.debug(f"Executing rental heatmap query: {query}")
        df = pd.read_sql(query, engine)
        if len(df) == 0:
            logger.warning("No rental heatmap data found")
            raise HTTPException(status_code=404, detail="No rental heatmap data found")
        logger.info(f"Rental heatmap query executed successfully. Row count: {len(df)}")
        ...
        logger.debug(f"Rental heatmap API response: {response}")
        return response
    except Exception as e:
        error_msg = f"Error fetching rental heatmap: {str(e)}"
        logger.error(error_msg, exc_info=True)
        raise HTTPException(status_code=500, detail=error_msg)
